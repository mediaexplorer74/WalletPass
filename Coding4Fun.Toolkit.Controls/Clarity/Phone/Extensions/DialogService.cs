...
namespace Clarity.Phone.Extensions
{
  public class DialogService
  {
    ...
    internal PhoneApplicationPage Page => this._page ?? (this._page = ((FrameworkElement) this.RootFrame).GetFirstLogicalChildByType<PhoneApplicationPage>(false));
    ...
    private void InitializePopup()
    {
      ...
      else
      {
        ...
        ((FrameworkElement) this.RootFrame).Loaded += new RoutedEventHandler(this.RootFrameDeferredShowLoaded);
      }
    }
    ...
    {
      Grid grid1 = new Grid();
      ((FrameworkElement) grid1).Name = Guid.NewGuid().ToString();
      Grid grid2 = grid1;
    ...
    internal void CalculateVerticalOffset() => this.CalculateVerticalOffset((Panel) this._childPanel);
    ...
    private async void RunShowStoryboard(UIElement grid, DialogService.AnimationTypes animation) => this.RunShowStoryboard(grid, animation, TimeSpan.MinValue);
      ...
      TimeSpan delay)
    {
      ...
      ((DependencyObject) this.Page).Dispatcher.BeginInvoke((Action) (() =>
      {
        foreach (Timeline child in (PresentationFrameworkCollection<Timeline>) storyboard.Children)
    ...
    private void RunHideStoryboard(Grid grid, DialogService.AnimationTypes animation)
    {
      ...
      try
      {
        if (storyboard == null)
          return;
        ((Timeline) storyboard).Completed += new EventHandler(this.HideStoryboardCompleted);
        foreach (Timeline child in (PresentationFrameworkCollection<Timeline>) storyboard.Children)
          Storyboard.SetTarget(child, (DependencyObject) grid);
        storyboard.Begin();
      }
    ...
    private void HideStoryboardCompleted(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        ((PresentationFrameworkCollection<UIElement>) ((Panel) this._childPanel).Children).Clear();
      }
      catch
      {
      }
