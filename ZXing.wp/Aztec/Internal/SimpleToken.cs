...
namespace ZXing.Aztec.Internal
{
  public sealed class SimpleToken : Token
  {
    ...
    public override void appendTo(BitArray bitArray, byte[] text) => bitArray.appendBits((int) this.value, (int) this.bitCount);

    public override string ToString() => '<'.ToString() + SupportClass.ToBinaryString((int) this.value & (1 << (int) this.bitCount) - 1 | 1 << (int) this.bitCount | 1 << (int) this.bitCount).Substring(1) + (object) '>';
  }
}
