...
namespace ZXing.Aztec
{
  ...
  public class AztecEncodingOptions : EncodingOptions
  {
    ...
    {
      get => this.Hints.ContainsKey(EncodeHintType.ERROR_CORRECTION) ? new int?((int) this.Hints[EncodeHintType.ERROR_CORRECTION]) : new int?();
      set
      {
        if (!value.HasValue)
        {
    ...
    public int? Layers
    {
      get => this.Hints.ContainsKey(EncodeHintType.AZTEC_LAYERS) ? new int?((int) this.Hints[EncodeHintType.AZTEC_LAYERS]) : new int?();
      set
      {
        if (!value.HasValue)
        {
