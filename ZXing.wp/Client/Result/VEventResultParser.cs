// ZXing.Client.Result.VEventResultParser
...
namespace ZXing.Client.Result
{
  ...
  internal sealed class VEventResultParser : ResultParser
  {
    public override ParsedResult parse(ZXing.Result result)
    {
      ...
      string description = VEventResultParser.matchSingleVCardPrefixedField(
      ...
      else
      {
        ...
        if (!double.TryParse(str.Substring(0, length), NumberStyles.Float, (IFormatProvider) CultureInfo.InvariantCulture, out result1))
          return (ParsedResult) null;
        if (!double.TryParse(str.Substring(length + 1), NumberStyles.Float, 
            (IFormatProvider) CultureInfo.InvariantCulture, out result2))
          return (ParsedResult) null;
      }
      try
      {
        return (ParsedResult) new CalendarParsedResult(summary, startString, 
            endString, durationString, location, organizer, attendees, description, result1, result2);
      }
