...
namespace ZXing.Client.Result
{
  ...
  {
    private static readonly Regex RFC2445_DURATION = new Regex("\\A(?:P(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?)\\z", (RegexOptions) 8);
    ...
    private static readonly Regex DATE_TIME = new Regex("\\A(?:[0-9]{8}(T[0-9]{6}Z?)?)\\z", (RegexOptions) 8);
