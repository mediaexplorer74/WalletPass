...
namespace ZXing.QrCode.Internal
{
  ...
  public sealed class AlignmentPattern : ResultPoint
  {
    ...
    /// <returns></returns>
    internal AlignmentPattern combineEstimate(float i, float j, float newModuleSize) => new AlignmentPattern((float) (((double) this.X + (double) j) / 2.0), (float) (((double) this.Y + (double) i) / 2.0), (float) (((double) this.estimatedModuleSize + (double) newModuleSize) / 2.0));
  }
}
