...
// Type: ControlTiltEffect.TiltEffect
...
namespace ControlTiltEffect
{
  public class TiltEffect : DependencyObject
  {
    ...
    static TiltEffect()
    {
      ...
      TiltEffect.UseLogarithmicEase = false;
    }
      ...
      DependencyPropertyChangedEventArgs args)
    {
      ...
      if ((bool) args.NewValue)
        ((UIElement) (target as FrameworkElement)).ManipulationStarted += new EventHandler<ManipulationStartedEventArgs>(TiltEffect.TiltEffect_ManipulationStarted);
      else
        ((UIElement) (target as FrameworkElement)).ManipulationStarted -= new EventHandler<ManipulationStartedEventArgs>(TiltEffect.TiltEffect_ManipulationStarted);
    }
    ...
    private static void TryStartTiltEffect(FrameworkElement source, ManipulationStartedEventArgs e)
    {
      foreach (FrameworkElement visualAncestor in (((RoutedEventArgs) e).OriginalSource as FrameworkElement).GetVisualAncestors())
      {
        ...
        {
          if (tiltableItem.IsAssignableFrom(visualAncestor.GetType()) && !(bool) ((DependencyObject) visualAncestor).GetValue(TiltEffect.SuppressTiltProperty))
          {
      ...
      FrameworkElement container)
    {
      ...
      Point point2 = !(container is PhoneApplicationFrame) ? new Point(container.ActualWidth / 2.0, container.ActualHeight / 2.0) : (((container as PhoneApplicationFrame).Orientation & 2) != 2 ? new Point(container.ActualWidth / 2.0, container.ActualHeight / 2.0) : new Point(container.ActualHeight / 2.0, container.ActualWidth / 2.0));
